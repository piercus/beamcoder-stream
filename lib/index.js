...
const createDemuxerReadStream = function (options, streamOptions = {}) {
	...
	const {reconnectEvery = 0} = streamOptions;

	let lastRead = new Date();
	let currentDemuxer;
	// Reconnect Mechanism is build to avoid
	// [rtsp @ 0x7fe01c000900] line='RTSP/1.0 454 Session Not Found'
	// to see it, please set beamcoder.logging
	const resetDemuxer = function () {
		currentDemuxer = optionsPromise.then(o => beamcoder.demuxer(o));
		return currentDemuxer;
	};

	resetDemuxer();

	const getDemuxer = function () {
		if (reconnectEvery === 0) {
			return currentDemuxer;
		}

		const date = new Date();
		if (lastRead === null || ((date.getTime() - lastRead.getTime()) < reconnectEvery)) {
			lastRead = date;
			return currentDemuxer;
		}

		// Return currentDemuxer.then(dm => {
		// dm.forceClose()
		lastRead = date;
		return resetDemuxer();
		// })
	...
	const stream = new Readable({...streamOptions, objectMode: true,
		construct(cb) {
			getDemuxer().then(() => cb(), error => cb(error));
		},
		read() {
			getDemuxer()
				.then(dm => dm.read())
